...
document.addEventListener("DOMContentLoaded", async () => {
  // --- نمایش پیام به محض باز شدن فرم ---
  setStatus("لطفاً GPS خود را برای ثبت تردد فعال نمایید.");
  setTimeout(() => {
    setStatus(""); 
  }, 3000);
  // ------------------------------------
...
function getLocationWithWatch(waitMs){
  return new Promise(resolve=>{
    ...
    const watchId=navigator.geolocation.watchPosition(
      ...
      ()=>{},
      {enableHighAccuracy:true,maximumAge:30000,timeout:waitMs}
    );
...
function setStatus(m){$("captureStatus")&&( $("captureStatus").textContent=m );}
function setSyncStatus(m){$("syncStatus")&&( $("syncStatus").textContent=m );}
...
function getTime(d){
  return new Intl.DateTimeFormat("fa-IR",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false}).format(d);
}
