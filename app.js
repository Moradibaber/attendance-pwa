...
const ONLINE_TIME_SHEET_NAME = "OnlineTime"; // NEW: sheet that stores accumulated online time per user/day
...
// NEW: if the gap between two heartbeats is bigger than this, treat the user as
// having gone offline in between (so that gap isn't counted as "online" time).
const HEARTBEAT_GAP_THRESHOLD_MS = 90 * 1000; // 90 seconds - ~3x the client's 30s heartbeat interval
...
// NEW: headers for the OnlineTime sheet
const ONLINE_TIME_HEADERS = [
  "PersonnelCode",
  "FullName",
  "RecordDate",
  "FirstSeen",
...
function doGet(e) {
  if (e && e.parameter) {
    ...
    // NEW: fetch accumulated online time for a personnel (optionally for one date)
    if (e.parameter.action === "getOnlineTime") {
      return handleGetOnlineTime_(e);
    }
  }
...
function doPost(e) {
  ...
  try {
    ...
    // NEW: heartbeat pings sent periodically while the user's page is open/online
    if (type === "Heartbeat") {
      return handleHeartbeat(ss, data);
    }
...
/* =========================
   ...
   -------------------------------------------------
   Client is expected to POST, roughly every 20-60s
   while its tab/app is open and online:
     {
       type: "Heartbeat",
       personnelCode: "1234",
       firstName: "...",
   ...
   Each ping either extends the current online session
   (if it arrived within HEARTBEAT_GAP_THRESHOLD_MS of the
   previous one) or starts a new session (if the gap is
   bigger - meaning the user was offline / closed the tab
   in between). TotalOnlineSeconds accumulates per day.
========================= */

function handleHeartbeat(ss, data) {
  var sh = ss.getSheetByName(ONLINE_TIME_SHEET_NAME);
  if (!sh) {
    sh = ss.insertSheet(ONLINE_TIME_SHEET_NAME);
  }
  ensureOnlineTimeHeaders_(sh);
  ...
  var now = new Date();
  var recordDate = s(data.recordDate).trim();
  if (!recordDate) {
    recordDate = Utilities.formatDate(now, Session.getScriptTimeZone(), "yyyy-MM-dd");
  }

  var lastRow = sh.getLastRow();
  var rowIndex = findOnlineTimeRow_(sh, personnelCode, recordDate, lastRow);

  var totalSeconds = 0;
  var sessionsCount = 1;

  if (rowIndex === -1) {
    // First heartbeat of the day for this user -> new row / new session
    var newRow = [
      personnelCode,
      fullName,
      recordDate,
      ...
      now,          // LastSeen
      now,          // LastHeartbeatAt
      0,            // TotalOnlineSeconds
      formatSecondsToHMS_(0), // TotalOnlineHMS
      1             // SessionsCount
    ];
    ...
    var lastHeartbeatAt = sh.getRange(rowIndex, 6).getValue();
    var lastHeartbeatMs = lastHeartbeatAt instanceof Date ? lastHeartbeatAt.getTime() : parseDateToMs(lastHeartbeatAt);

    totalSeconds = Number(sh.getRange(rowIndex, 7).getValue()) || 0;
    sessionsCount = Number(sh.getRange(rowIndex, 9).getValue()) || 1;

    var gapMs = (lastHeartbeatMs !== null && !isNaN(lastHeartbeatMs)) ? (now.getTime() - lastHeartbeatMs) : null;

    if (gapMs !== null && gapMs >= 0 && gapMs <= HEARTBEAT_GAP_THRESHOLD_MS) {
      // Continuous session - add the elapsed gap to the running total
      totalSeconds += Math.round(gapMs / 1000);
    } else {
      // Gap too large (or unknown) - user was offline in between, start a new session
      sessionsCount += 1;
    }
    ...
    sh.getRange(rowIndex, 5).setValue(now); // LastSeen
    sh.getRange(rowIndex, 6).setValue(now); // LastHeartbeatAt
    sh.getRange(rowIndex, 7).setValue(totalSeconds);
    sh.getRange(rowIndex, 8).setValue(formatSecondsToHMS_(totalSeconds));
    sh.getRange(rowIndex, 9).setValue(sessionsCount);
    sh.getRange(rowIndex, 5, 1, 2).setNumberFormat("yyyy-mm-dd hh:mm:ss");
  }

  return jsonOut({
    ok: true,
    sheet: ONLINE_TIME_SHEET_NAME,
    row: rowIndex,
    personnelCode: personnelCode,
    recordDate: recordDate,
    totalOnlineSeconds: totalSeconds,
    totalOnlineHMS: formatSecondsToHMS_(totalSeconds),
    sessionsCount: sessionsCount
  });
}

function ensureOnlineTimeHeaders_(sh) {
  if (sh.getLastRow() === 0) {
    sh.getRange(1, 1, 1, ONLINE_TIME_HEADERS.length).setValues([ONLINE_TIME_HEADERS]);
    sh.getRange(1, 1, 1, ONLINE_TIME_HEADERS.length).setFontWeight("bold");
    sh.setFrozenRows(1);
  }
}

// Finds the row for a given personnelCode + recordDate pair. Returns -1 if not found.
function findOnlineTimeRow_(sh, personnelCode, recordDate, lastRow) {
  if (lastRow < 2) return -1;

  var values = sh.getRange(2, 1, lastRow - 1, 3).getValues(); // PersonnelCode, FullName, RecordDate
  for (var i = 0; i < values.length; i++) {
    if (s(values[i][0]).trim() === personnelCode && s(values[i][2]).trim() === recordDate) {
      return i + 2;
    }
  }
  return -1;
...
function formatSecondsToHMS_(totalSeconds) {
...
// doGet action=getOnlineTime&personnelCode=...&recordDate=YYYY-MM-DD (recordDate optional)
function handleGetOnlineTime_(e) {
  try {
    var personnelCode = stringifyOrBlank(e && e.parameter ? e.parameter.personnelCode : "");
    var recordDate = stringifyOrBlank(e && e.parameter ? e.parameter.recordDate : "");

    if (!personnelCode) {
      return jsonResponse({ ok: false, error: "PersonnelCode الزامی است." });
    }

    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sh = ss.getSheetByName(ONLINE_TIME_SHEET_NAME);
    if (!sh) {
      return jsonResponse({ ok: true, personnelCode: personnelCode, days: [] });
    }
    ...
    var values = sh.getRange(2, 1, lastRow - 1, ONLINE_TIME_HEADERS.length).getValues();
    var days = [];

    for (var i = 0; i < values.length; i++) {
      var row = values[i];
      if (s(row[0]).trim() !== personnelCode) continue;
      if (recordDate && s(row[2]).trim() !== recordDate) continue;

      days.push({
        recordDate: row[2],
        firstSeen: row[3],
        lastSeen: row[4],
        totalOnlineSeconds: row[6],
        totalOnlineHMS: row[7],
        sessionsCount: row[8]
      });
    }
    ...
    return jsonResponse({ ok: true, personnelCode: personnelCode, days: days });
  } catch (err) {
...
// NEW: getter for the OnlineTime sheet, consistent with the other getX Sheet() helpers
function getOnlineTimeSheet() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName(ONLINE_TIME_SHEET_NAME);
  if (!sheet) sheet = ss.insertSheet(ONLINE_TIME_SHEET_NAME);
  ensureHeaders(sheet, ONLINE_TIME_HEADERS);
  return sheet;
}
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem("نمایش شیت OnlineTime", "showOnlineTimeSheet") // NEW
    .addSeparator()
...
// NEW
function showOnlineTimeSheet() {
  var sheet = getOnlineTimeSheet();
  sheet.showSheet();
  SpreadsheetApp.getActiveSpreadsheet().setActiveSheet(sheet);
}
...
/**
 * شیت را با نام مشخص شده دریافت می کند یا در صورت عدم وجود آن را ایجاد می کند.
 * @param {string} sheetName نام شیت مورد نظر.
 * @param {string[]} headers آرایه ای از هدرهایی که باید در صورت ایجاد شیت، در ردیف اول قرار گیرند.
 * @return {GoogleAppsScript.Spreadsheet.Sheet} شیتی که دریافت یا ایجاد شده است.
 */
function getOrCreateSheet_(sheetName, headers) {
  ...
  if (!sheet) {
    ...
    Logger.log("شیت '" + sheetName + "' ایجاد شد.");
  } else {
    var existingHeaders = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
    if (!existingHeaders || existingHeaders.join(',') !== headers.join(',')) {
      Logger.log("هدرهای شیت '" + sheetName + "' با هدرهای مورد انتظار مطابقت ندارند. لطفا هدرها را به صورت دستی تنظیم کنید.");
    }
  }
  return sheet;
}
...
function setupAttendance() {
  var userStatusHeaders = ["PersonnelCode", "LastStatus", "LastSeen", "ClockRisk"];
  var userStatusSheet = getOrCreateSheet_("UserStatus", userStatusHeaders);
}
